...
class Sequentialmodel(nn.Module):
    ...
    def __init__(self, layers, device):
        ...
        # print("self.L:", self.L)
        # print("self.W:",self.W)
        # print("torch.exp(self.G * self.x_k):", torch.diag(torch.exp(self.G * self.x_k).squeeze(1)))
    ...
    def fun_f(self, x_t):
        ...
        temp_l = a_l ** self.alpha1 * torch.exp(-x) * result1 - \
                  a_l * torch.exp(-self.G * x - self.G + 1) * torch.pow(1 + x, (-k)) + \
                 k * torch.exp(-self.G * x - self.G + 1) * torch.pow(1 + x, -k-1)
        left = a_l ** self.alpha1 * torch.exp(-x) - 1 / math.gamma(self.s - self.alpha1) * temp_l


        temp_r = a_r ** self.alpha1 * torch.exp(-x) * result2 - \
                 a_r * torch.exp(self.M * x - self.M - 1) * torch.pow(1 - x, (-k)) + \
                 k * torch.exp(self.M * x - self.M - 1) * torch.pow(1 - x, -k-1)
        right = a_r ** self.alpha1 * torch.exp(-x) - 1 / math.gamma(self.s - self.alpha1) * temp_r


        f = -torch.exp(-x) * torch.sin(t) - (left + right)*torch.cos(t)
