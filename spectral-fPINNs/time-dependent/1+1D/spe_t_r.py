...
class Sequentialmodel(nn.Module):
    ...
    def fra_term(self, x):
        ...
        fra_jacobi = torch.reshape(self.jacobi_polynomial(1 - self.alpha1, self.alpha1, x), [self.N + 1, -1])[:self.N,:]

        Dr_n_result = self.coe2 * fra_jacobi @ torch.diag(torch.pow(1 - x, 1 - self.alpha1).squeeze(1))
    ...
    def a_j(self, t):
        ...
        x_k_t = torch.cartesian_prod(self.x_k.squeeze(1), t.squeeze(1))##笛卡尔积【(x_0,t_0),(x_0,t_1),
    ...
    def fun_f(self, x_t):
        ...
        f = -torch.exp(-x)*torch.sin(t) -(right - pow(self.M, self.alpha1) * torch.exp(-x))*torch.cos(t)
    ...
    def PDE(self, x_t):
        ...
        D_r = self.compute(x_t)
        L_v =  V_t - D_r + pow(self.M, self.alpha1) * V - f
